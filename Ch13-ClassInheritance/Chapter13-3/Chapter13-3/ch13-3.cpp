...
int main() {
    ...
    char templabel[LEN], tempcolor[LEN], tempstyle[LEN];
	char choice;	//what we choose
    int temprating=0;

    ABC* pt[ITEMS]; // array of abstract base class pointers

    for(int i=0; i<ITEMS; i++) {
        cout<<"\nEnter 1 for Shirt, 2 for Balloon, 3 for Map: ";
        while ((cin>>choice).get() && (choice!='1' && choice!='2' && choice!='3')) 
